...
/**
 * @file main.c
 * @brief Native GTK4 MBLINK product shell.
 *
 * The desktop layer renders the portable workspace and Mercedes profile but
 * deliberately owns no diagnostic protocol state. Transport-dependent values
 * remain unavailable until a Linux provider supplies real vehicle data.
 */
...
static const char mblink_css[] =
    "window, .mblink-root { background: #050608; color: #e8ecef; }"
    ".mblink-sidebar { background: #0a0c0f; border-right: 1px solid #34383d; }"
    ".mblink-brand { padding: 20px 18px 18px 18px; background: linear-gradient(135deg, #15191e, #090b0e); border-bottom: 1px solid #34383d; }"
    ".mblink-title { font-size: 30px; font-weight: 900; letter-spacing: 3px; color: #eef1f3; }"
    ".mblink-kicker { font-size: 10px; font-weight: 800; letter-spacing: 2px; color: #b9c0c6; }"
    ".mblink-muted { color: #7e858c; }"
    ".mblink-value { color: #eef1f3; font-weight: 700; }"
    ".mblink-sidebar list { background: transparent; }"
    ".mblink-sidebar row { margin: 5px 10px; border-radius: 12px; border: 1px solid transparent; }"
    ".mblink-sidebar row:hover { background: #15191e; border-color: #34383d; }"
    ".mblink-sidebar row:selected { background: #1a1f24; border-color: #8d969e; }"
    ".mblink-card { background: linear-gradient(135deg, #171b20, #0d1014); border: 1px solid #353a40; border-radius: 18px; padding: 22px; }"
    ".mblink-card-title { font-size: 20px; font-weight: 800; color: #eef1f3; }"
    ".mblink-card-kicker { font-size: 10px; font-weight: 800; letter-spacing: 2px; color: #8c949b; }"
    ".mblink-card-summary { font-size: 15px; color: #bdc4ca; }"
    ".mblink-chip { padding: 7px 11px; border-radius: 999px; border: 1px solid #3b4147; background: #111419; color: #dfe4e8; font-size: 11px; font-weight: 700; }"
    ".mblink-warning { color: #d19e47; border-color: #72572f; }"
    ".mblink-success { color: #63ab7c; border-color: #365f45; }"
    ".mblink-rule { background: #353a40; }"
    ".mblink-workspace-title { font-weight: 700; color: #e7ebee; }"
    ".mblink-workspace-summary { color: #7f878e; }"
    ".mblink-command-note { color: #8d959c; font-size: 12px; }"
    ".mblink-about-button { margin: 8px 14px 14px 14px; }";
...
static GtkWidget *make_card(const char *kicker, const char *title)
{
    ...
    if (kicker != NULL) {
        gtk_box_append(GTK_BOX(card), make_left_label(kicker, "mblink-card-kicker"));
    }
    if (title != NULL) {
        gtk_box_append(GTK_BOX(card), make_left_label(title, "mblink-card-title"));
    }
    return card;
}
...
static void append_vehicle_workspace(MblinkLinuxApp *app)
{
    ...
    append_detail(identity, "Engine", profile != NULL ? profile->engine_family : "Unavailable");
    append_detail(identity, "Profile", profile != NULL ? profile->display_name : "Unavailable");
    append_detail(identity, "Engine ECU", endpoint != NULL ? endpoint->name : "Unavailable");
    append_detail(identity, "Definition", endpoint != NULL ? mblink_mercedes_definition_status_name(endpoint->status) : "Unavailable");
    append_detail(identity, "Physical CAN", "0x7E0 → 0x7E8");

    gtk_box_append(GTK_BOX(connection), make_chip("TRANSPORT PROVIDER NOT CONFIGURED", "mblink-warning"));
    gtk_box_append(GTK_BOX(connection), make_left_label(
        "The portable diagnostic profile is live. A Linux BLE, serial or SocketCAN provider is still required before this front end can connect to a vehicle.",
        "mblink-command-note"));
...
static void append_modules_workspace(MblinkLinuxApp *app)
{
    ...
    if (profile == NULL || profile->endpoint_count == 0U) {
        gtk_box_append(GTK_BOX(card), make_left_label("No endpoint definitions available", "mblink-command-note"));
    } else {
        for (index = 0U; index < profile->endpoint_count; ++index) {
            ...
            append_detail(card, endpoint->name, mblink_mercedes_definition_status_name(endpoint->status));
        }
    }
...
static void append_faults_workspace(MblinkLinuxApp *app)
{
    ...
    gtk_box_append(GTK_BOX(mercedes), make_chip("NOT SCANNED · TRANSPORT OFFLINE", "mblink-warning"));
    gtk_box_append(GTK_BOX(mercedes), make_left_label(
        "The shared C core includes the read-only UDS ReadDTCInformation codec. No result is invented while the Linux transport is absent.",
        "mblink-command-note"));
    gtk_box_append(GTK_BOX(obd), make_chip("NOT SCANNED · TRANSPORT OFFLINE", "mblink-warning"));
...
{
    GtkWidget *card = make_card(compact ? "PORTABLE PARAMETER TABLE" : "SHARED PARAMETER CATALOGUE",
...
static void append_dashboard_workspace(MblinkLinuxApp *app)
{
    ...
    gtk_box_append(GTK_BOX(card), make_chip("WAITING FOR LIVE TRANSPORT", "mblink-warning"));
...
static void append_graphs_workspace(MblinkLinuxApp *app)
{
    ...
    gtk_box_append(GTK_BOX(card), make_chip("NO LIVE SAMPLE HISTORY", "mblink-warning"));
    gtk_box_append(GTK_BOX(card), make_left_label(
        "Graph definitions and bounded history are owned by the portable parameter and telemetry layers. Traces render after a Linux transport supplies real samples.",
        "mblink-command-note"));
    gtk_box_append(GTK_BOX(app->content_body), card);
}
...
static void append_log_workspace(MblinkLinuxApp *app)
{
    ...
    gtk_box_append(GTK_BOX(card), make_chip("NO ACTIVE SESSION", "mblink-warning"));
    gtk_box_append(GTK_BOX(card), make_left_label(
        "The shared recorder preserves raw command/response evidence and telemetry. Linux does not present fabricated session data while its transport edge is unavailable.",
        "mblink-command-note"));
    gtk_box_append(GTK_BOX(app->content_body), card);
}
...
static void append_settings_workspace(MblinkLinuxApp *app)
{
    ...
    GtkWidget *manufacturer = make_card("MERCEDES CAPABILITY MAP", "Evidence-gated manufacturer targets");
    char target_count[32];

    (void)g_snprintf(target_count, sizeof(target_count), "%zu definitions", mblink_mercedes_om651_signal_count());
    append_detail(system, "Version", mblink_version());
    append_detail(system, "Application ID", "com.github.The-First-Infiltrator.MBLINK");
    append_detail(system, "Author", "Shannon Smith");
    append_detail(system, "Licence", "GPL-3.0-or-later");
    append_detail(system, "Portable core", mblink_self_check() ? "Validated" : "Invalid metadata");
...
static void render_workspace(MblinkLinuxApp *app, MblinkWorkspaceSection section)
{
    clear_box(app->content_body);
    switch (section) {
    case MBLINK_WORKSPACE_VEHICLE: append_vehicle_workspace(app); break;
    case MBLINK_WORKSPACE_MODULES: append_modules_workspace(app); break;
    case MBLINK_WORKSPACE_FAULTS: append_faults_workspace(app); break;
    case MBLINK_WORKSPACE_LIVE_DATA: append_parameter_catalogue(app, false); break;
    case MBLINK_WORKSPACE_TABLE: append_parameter_catalogue(app, true); break;
    case MBLINK_WORKSPACE_DASHBOARD: append_dashboard_workspace(app); break;
    case MBLINK_WORKSPACE_GRAPHS: append_graphs_workspace(app); break;
    case MBLINK_WORKSPACE_LOG: append_log_workspace(app); break;
    case MBLINK_WORKSPACE_SETTINGS: append_settings_workspace(app); break;
    case MBLINK_WORKSPACE_SECTION_COUNT: break;
    }
}
...
static GtkWidget *workspace_row(const MblinkWorkspaceSectionDescriptor *descriptor)
{
    ...
    GtkWidget *title = make_left_label(descriptor->title, "mblink-workspace-title");
    GtkWidget *summary = make_left_label(descriptor->summary, "mblink-workspace-summary");
    set_margins(box, 11);
    gtk_box_append(GTK_BOX(box), title);
    gtk_box_append(GTK_BOX(box), summary);
    gtk_list_box_row_set_child(GTK_LIST_BOX_ROW(row), box);
    g_object_set_data(G_OBJECT(row), "mblink-section", GINT_TO_POINTER((int)descriptor->section));
    return row;
}

static void update_content(MblinkLinuxApp *app, MblinkWorkspaceSection section)
{
    const MblinkWorkspaceSectionDescriptor *descriptor = mblink_workspace_section(section);
    if (descriptor == NULL) {
        return;
    }
...
static void row_selected(GtkListBox *list_box, GtkListBoxRow *row, gpointer user_data)
{
...
static void about_clicked(GtkButton *button, gpointer user_data)
{
    MblinkLinuxApp *app = user_data;
    (void)button;
    mblink_linux_show_about(app->window);
}
...
static GtkWidget *build_brand_header(void)
{
    ...
    GtkWidget *emblem = gtk_image_new_from_resource("/com/github/The-First-Infiltrator/MBLINK/mblink-emblem.png");
    GtkWidget *labels = gtk_box_new(GTK_ORIENTATION_VERTICAL, 3);
    GtkWidget *title = make_left_label("MBLINK", "mblink-title");
    GtkWidget *subtitle = make_left_label("MERCEDES DIAGNOSTIC COMMAND", "mblink-kicker");
    GtkWidget *target = make_left_label("C207 · OM651 · DELPHI CRD3.x", "mblink-muted");
...
static GtkWidget *build_sidebar(MblinkLinuxApp *app)
{
    ...
    GtkWidget *about_button = gtk_button_new_with_label("About MBLINK");
    size_t index;
    ...
    for (index = 0U; index < mblink_workspace_section_count(); ++index) {
        const MblinkWorkspaceSectionDescriptor *descriptor = mblink_workspace_section_at(index);
        if (descriptor != NULL) {
    ...
    gtk_widget_add_css_class(about_button, "mblink-about-button");
...
static GtkWidget *build_status_strip(void)
{
    ...
    gtk_box_append(GTK_BOX(strip), make_chip("LINUX TRANSPORT OFFLINE", "mblink-warning"));
    return strip;
}
...
static GtkWidget *build_content(MblinkLinuxApp *app)
{
    ...
    app->content_kicker = make_left_label("DIAGNOSTIC WORKSPACE", "mblink-card-kicker");
    app->content_title = make_left_label("Vehicle", "mblink-card-title");
    app->content_summary = make_left_label("Vehicle identity, adapter and connection information", "mblink-card-summary");
    ...
    gtk_scrolled_window_set_policy(GTK_SCROLLED_WINDOW(scroll), GTK_POLICY_NEVER, GTK_POLICY_AUTOMATIC);
    gtk_scrolled_window_set_child(GTK_SCROLLED_WINDOW(scroll), app->content_body);
    gtk_widget_set_vexpand(scroll, TRUE);
...
static void activate(GtkApplication *application, gpointer user_data)
{
    ...
    gtk_window_set_icon_name(GTK_WINDOW(window), "mblink");
