...
Unknown typed PIDs return `MBLINK_OBD2_RESULT_UNSUPPORTED_PID`; MBLINK does not invent formulas.
...
Mode `03`/`43`, `07`/`47` and `0A`/`4A` cover stored, pending and permanent DTCs. Zero padding is ignored, duplicates are removed and the bounded list reports overflow instead of truncating silently.

Mode 04 clearing requires both `confirmed` and `acknowledge_readiness_reset` in `MblinkObd2ClearAuthorization`. Higher layers must still require an explicit user action immediately before execution.
