...
The shared descriptor set currently covers 35 standard OBD-II values used by the capability-gated live scheduler, including distinct throttle-valve and accelerator-pedal channels. The 2026-08-31 C207 capture proved seven additional advertised scalar PIDs that are now exposed: engine run time (0x1F), distance with MIL on (0x21), oxygen-sensor-1 equivalence ratio (0x24), warm-ups since clear (0x30), distance since clear (0x31), catalyst temperature B1S2 (0x3E), and time run with MIL on (0x4D). `mblink_parameter_from_obd2()` converts a decoded `MblinkObd2Sample` transactionally into the shared model. Manufacturer-specific UDS definitions can use the same contract later without teaching SwiftUI or GTK protocol formulas.
...
MBLINK 0.7.102 also consumes LINK's responder-attributed Mode 01 history. When a
