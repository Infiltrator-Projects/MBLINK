...
The immediate evidence step is now two-track: preserve every additional physical C207/OM651 response as a sanitised regression fixture, and recover the post-VIN Whisper runtime configuration that maps DataID -> device/provider -> request/result -> extraction/formula. The backend/authentication and package-delivery path is already source-backed; more authentication reverse-engineering is no longer the priority. The raw ACS ZIP is transient and deleted after successful installation, so the highest-value historical artifacts are the app runtime `incoming`, `actionproviders/config`, `deviceproviders/config`, `vinmapping/config` and VIN-derived `_configs` content. `tools/mblink-whisper-recover.py` performs a bounded read-only search for those artifacts, suppresses the known APK bootstrap parameterisation, and reconstructs exact DataID -> device/provider -> request/result -> extraction/formula bindings from any retained Whisper properties as `source-backed-candidate` evidence. Once a source-backed mapping exists, the C207 capture becomes validation and promotion evidence rather than a blind DID search.
