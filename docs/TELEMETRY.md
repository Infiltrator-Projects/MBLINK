...
The current iPhone bridge keeps its stream in memory for sharing. Manual disconnect/reconnect attempts append distinct session boundaries to the same CSV stream for the lifetime of the controller, so a later successful connection does not erase an earlier failure. When VIN discovery completes, the current session header is rewritten with that VIN before export. Durable file-backed storage across application termination remains a later product hardening task, not a change to the portable recorder contract.
